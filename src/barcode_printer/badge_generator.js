...
function createBadgePdf(badgeData, doneWritingPdf) {
  ...
  const image_path1 = badgeData.schoolLogoPath;
  doc.image(image_path1, 3, 3, {fit: [30, 30]}).stroke();
  ...
  const image_path2 = badgeData.studentImagePath;
  doc.image(image_path2, 30, 65, {
    fit: [95, 95]}
    ).stroke();
  ...
  // show the barcode
  doc.image(badgeData.barcodeImagePath,  25,  205, {fit: [175, 30]}).stroke();
  ...
  // write to a file
  pdfOutputPath = appRoot + '/output/test_file2.pdf'
